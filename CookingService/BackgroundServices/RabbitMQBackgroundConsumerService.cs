...
using CookingService.Utility;
using Newtonsoft.Json;
using RabbitMQ.Client;
using RabbitMQ.Client.Events;
using System.Text;

namespace CookingService.BackgroundServices
{
    public class RabbitMQBackgroundConsumerService : BackgroundService
    {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += (ch, ea) =>
            {
                ...
                var orderItem = JsonConvert.DeserializeObject<OrderItem>(message);

                //Stored Offer Details into the Database
                using (var scope = serviceScopeFactory.CreateScope())
                {
                    var _dbContext = scope.ServiceProvider.GetRequiredService<CookingDbContext>();
